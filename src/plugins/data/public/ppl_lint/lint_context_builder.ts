...
export function buildPPLLintContext(
  ...
  //  - An Open Distro engine (e.g. Elasticsearch) never runs Calcite whatever
  //    version it reports, and no backend setting can change that.
  //  - Otherwise prefer backend-reported settings, because the version heuristic
  //    cannot see an admin-disabled Calcite on a >= 3.3 cluster.
  const engineDerived = deriveIsCalcite(effectiveVersion, engineType);
  const isCalcite =
    engineDerived === false
      ? false
      : cachedSettings
