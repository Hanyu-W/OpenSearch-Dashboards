...
// Re-exported so the autocomplete test (and any future caller) keeps importing
// the space-token resolver from this module. The implementation now lives in
// runtime_grammar_utils so autocomplete, validation, and lint never disagree.
...
/**
 * Pick the start rule for C3/parsing based on query shape. Delegates to the
 * shared {@link pickRuntimeStartRuleIndex} (in runtime_grammar_utils) with the
 * `subPipeline` fallback enabled: unlike validation/lint, autocomplete always
 * wants `subPipeline` as a last resort when a grammar lacks both
 * `pipeStartRuleIndex` and `commands`, so it can still offer pipeline
 * completions. (The shared helper defaults that fallback off for the other two
 * surfaces.)
 */
