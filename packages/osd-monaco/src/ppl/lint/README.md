...
|---|---|---|
| *(none)* | Pure query-text analysis | worker |
...
is the *effective* shipped default: the catalog marks every rule
...
1. **Catalog entry** in `rules_catalog.json` — id, severity, message,
   `docUrl`, `appliesTo`, context flags.
...
4. **Hover content** in `hover/engine_outcomes.ts` (what the engine actually
   does, shown on hover).
5. **Doc-links snapshot** parity (`__tests__/doc_links.test.ts`).
6. **Per-rule doc file** in `docs/rules/` with the five required sections
   (enforced by `__tests__/docs_parity.test.ts`).
7. **Tests** in `__tests__/` — including a negative case (query that must NOT
   fire).
...
- **Markers** — `diagnostic_to_marker.ts` sets
  ...
  clickable link.
- **Hover cards** — `hover/hover_card.ts` renders message + engine outcome +
  facts + "Learn more"; content per rule in `hover/engine_outcomes.ts`.
